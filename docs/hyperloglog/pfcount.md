...
当 [PFCOUNT key [key …]](#pfcount) 命令作用于单个键时，
返回储存在给定键的 HyperLogLog 的近似基数，
如果键不存在，
那么返回 `0` 。

当 [PFCOUNT key [key …]](#pfcount) 命令作用于多个键时，
...
一个程序可以在每次执行搜索查询时调用一次 [PFADD key element [element …]](pfadd.md#pfadd) ，
并通过调用 [PFCOUNT key [key …]](#pfcount) 命令来获取这个记录的近似结果。
