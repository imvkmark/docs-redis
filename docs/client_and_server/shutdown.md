...
* 停止所有客户端
* 如果有至少一个保存点在等待，执行 [SAVE](../persistence/save.md#save) 命令
* 如果 AOF 选项被打开，更新 AOF 文件
* 关闭 redis 服务器(server)

如果持久化被打开的话， [SHUTDOWN](#shutdown) 命令会保证服务器正常关闭而不丢失任何数据。

另一方面，假如只是单纯地执行 [SAVE](../persistence/save.md#save) 命令，然后再执行 [QUIT](quit.md#quit) 命令，则没有这一保证 —— 因为在执行 [SAVE](../persistence/save.md#save) 之后、执行 [QUIT](quit.md#quit) 之前的这段时间中间，其他客户端可能正在和服务器进行通讯，这时如果执行 [QUIT](quit.md#quit) 就会造成数据丢失。
