...
* AOF 文件有序地保存了对数据库执行的所有写入操作，
  ...
  举个例子，
  如果你不小心执行了 [FLUSHALL](../database/flushall.md#flushall) 命令，
  但只要 AOF 文件未被重写，
  那么只要停止服务器，
  移除 AOF 文件末尾的 [FLUSHALL](../database/flushall.md#flushall) 命令，
  并重启 Redis ，
  就可以将数据集恢复到 [FLUSHALL](../database/flushall.md#flushall) 执行之前的状态。
...
* AOF 在过去曾经发生过这样的 bug ：
  ...
  （举个例子，阻塞命令 [BRPOPLPUSH source destination timeout](../list/brpoplpush.md#brpoplpush) 就曾经引起过这样的 bug 。）
  测试套件里为这种情况添加了测试：
  它们会自动生成随机的、复杂的数据集，
  并通过重新载入这些数据来确保一切正常。
...
你也可以通过调用 [SAVE](../persistence/save.md#save) 或者 [BGSAVE](../persistence/bgsave.md#bgsave) ，
手动让 Redis 进行数据集保存操作。
...
从现在开始，
每当 Redis 执行一个改变数据集的命令时（比如 [SET key value [EX seconds] [PX milliseconds] [NX|XX]](../string/set.md#set)），
...
举个例子，
如果你对一个计数器调用了 100 次 [INCR key](../string/incr.md#incr) ，
那么仅仅是为了保存这个计数器的当前值，
AOF 文件就需要使用 100 条记录（entry）。

然而在实际上，
只使用一条 [SET key value [EX seconds] [PX milliseconds] [NX|XX]](../string/set.md#set) 命令已经足以保存计数器的当前值了，
其余 99 条记录实际上都是多余的。
...
执行 [BGREWRITEAOF](../persistence/bgrewriteaof.md#bgrewriteaof) 命令，
Redis 将生成一个新的 AOF 文件，
这个文件包含重建当前数据集所需的最少命令。

Redis 2.2 需要自己手动执行 [BGREWRITEAOF](../persistence/bgrewriteaof.md#bgrewriteaof) 命令；
...
[BGSAVE](../persistence/bgsave.md#bgsave) 执行的过程中，
不可以执行 [BGREWRITEAOF](../persistence/bgrewriteaof.md#bgrewriteaof) 。
反过来说，
在 [BGREWRITEAOF](../persistence/bgrewriteaof.md#bgrewriteaof) 执行的过程中，
也不可以执行 [BGSAVE](../persistence/bgsave.md#bgsave) 。

这可以防止两个 Redis 后台进程同时对磁盘进行大量的 I/O 操作。

如果 [BGSAVE](../persistence/bgsave.md#bgsave) 正在执行，
并且用户显示地调用 [BGREWRITEAOF](../persistence/bgrewriteaof.md#bgrewriteaof) 命令，
那么服务器将向用户回复一个 `OK` 状态，
并告知用户，
[BGREWRITEAOF](../persistence/bgrewriteaof.md#bgrewriteaof) 已经被预定执行：
一旦 [BGSAVE](../persistence/bgsave.md#bgsave) 执行完毕，
[BGREWRITEAOF](../persistence/bgrewriteaof.md#bgrewriteaof) 就会正式开始。
...
* 传送快照可以使用 SCP 来完成（SSH 的组件）。
  ...
  并将这个 key 添加到 VPS 的 authorized\_keys 文件中，
