...
* 复制功能可以单纯地用于数据冗余（data redundancy），
  ...
  比如说，
  繁重的 [SORT key [BY pattern] [LIMIT offset count] [GET pattern [GET pattern …]] [ASC | DESC] [ALPHA] [STORE destination]](../database/sort.md#sort) 命令可以交给附属节点去运行。
...
无论是初次连接还是重新连接，
当建立一个从服务器时，
从服务器都将向主服务器发送一个 [SYNC](../internal/sync.md#sync) 命令。

接到 [SYNC](../internal/sync.md#sync) 命令的主服务器将开始执行 [BGSAVE](../persistence/bgsave.md#bgsave) ，
并在保存操作执行期间，
将所有新执行的写入命令都保存到一个缓冲区里面。

当 [BGSAVE](../persistence/bgsave.md#bgsave) 执行完毕后，
...
然后向它发送 [SYNC](../internal/sync.md#sync) 命令，
过一阵子，
...
即使有多个从服务器同时向主服务器发送 [SYNC](../internal/sync.md#sync) ，
主服务器也只需执行一次 [BGSAVE](../persistence/bgsave.md#bgsave) 命令，
就可以处理所有这些从服务器的同步请求。
...
Redis 2.8 的这个部分重同步特性会用到一个新增的 [PSYNC master\_run\_id offset](../internal/psync.md#psync) 内部命令，
而 Redis 2.8 以前的旧版本只有 [SYNC](../internal/sync.md#sync) 命令，
不过，
只要从服务器是 Redis 2.8 或以上的版本，
它就会根据主服务器的版本来决定到底是使用 [PSYNC master\_run\_id offset](../internal/psync.md#psync) 还是 [SYNC](../internal/sync.md#sync) ：

* 如果主服务器是 Redis 2.8 或以上版本，那么从服务器使用 [PSYNC master\_run\_id offset](../internal/psync.md#psync) 命令来进行同步。
* 如果主服务器是 Redis 2.8 之前的版本，那么从服务器使用 [SYNC](../internal/sync.md#sync) 命令来进行同步。
...
另外一种方法是调用 [SLAVEOF host port](../replication/slaveof.md#slaveof) 命令，
输入主服务器的 IP 和端口，
然后同步就会开始：
...
也可以通过 [CONFIG SET parameter value](../configure/config_set.md#config-set) 命令来开启或关闭这个模式。
