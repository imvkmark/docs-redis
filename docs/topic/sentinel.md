...
* `down-after-milliseconds` 选项指定了 Sentinel 认为服务器已经断线所需的毫秒数。
  ...
  如果服务器在给定的毫秒数之内，
  没有返回 Sentinel 发送的 [PING](../debug/ping.md#ping) 命令的回复，
  或者返回一个错误，
...
对向它发送 [PING](../debug/ping.md#ping) 命令的 Sentinel 返回一个有效回复（valid reply），
那么 Sentinel 就会将这个服务器标记为主观下线。

服务器对 [PING](../debug/ping.md#ping) 命令的有效回复可以是以下三种回复的其中一种：
...
又或者在指定时间内没有回复 [PING](../debug/ping.md#ping) 命令，
...
* 每个 Sentinel 以每秒钟一次的频率向它所知的主服务器、从服务器以及其他 Sentinel 实例发送一个 [PING](../debug/ping.md#ping) 命令。
* 如果一个实例（instance）距离最后一次有效回复 [PING](../debug/ping.md#ping) 命令的时间超过 `down-after-milliseconds` 选项所指定的值，
...
* 如果一个主服务器被标记为主观下线，
  ...
  那么这个主服务器被标记为客观下线。
* 在一般情况下，
  每个 Sentinel 会以每 10 秒一次的频率向它已知的所有主服务器和从服务器发送 [INFO [section]](../client_and_server/info.md#info) 命令。
  当一个主服务器被 Sentinel 标记为客观下线时，
  Sentinel 向下线主服务器的所有从服务器发送 [INFO [section]](../client_and_server/info.md#info) 命令的频率会从 10 秒一次改为每秒一次。
* 当没有足够数量的 Sentinel 同意主服务器已经下线，
  主服务器的客观下线状态就会被移除。
  当主服务器重新向 Sentinel 的 [PING](../debug/ping.md#ping) 命令返回有效回复时，
  主服务器的主管下线状态就会被移除。
...
* [PING](../debug/ping.md#ping) ：返回 `PONG` 。
...
你不可以使用 [PUBLISH channel message](../pubsub/publish.md#publish) 命令向这个服务器发送信息，
但你可以用 [SUBSCRIBE channel [channel …]](../pubsub/subscribe.md#subscribe) 命令或者 [PSUBSCRIBE pattern [pattern …]](../pubsub/psubscribe.md#psubscribe) 命令，
通过订阅给定的频道来获取相应的事件提醒。
...
* `+slave-reconf-sent <instance details>` ：领头（leader）的 Sentinel 向实例发送了 [SLAVEOF host port](../replication/slaveof.md#slaveof) 命令，为实例设置新的主服务器。
...
* 向已下线主服务器的从服务器发送 [SLAVEOF host port](../replication/slaveof.md#slaveof) 命令，
  让它们去复制新的主服务器。
...
* 在失效主服务器属下的从服务器当中，
  那些被标记为主观下线、已断线、或者最后一次回复 [PING](../debug/ping.md#ping) 命令的时间大于五秒钟的从服务器都会被淘汰。
* 在失效主服务器属下的从服务器当中，
...
比如说，
为了判断一个实例是否可用，
Sentinel 会记录这个实例最后一次相应 [PING](../debug/ping.md#ping) 命令的时间，
并将这个时间和当前时间进行对比，
...
会先向服务器发送一个 [SCRIPT KILL](../script/script_kill.md#script-kill) 命令，
如果服务器正在执行的是一个只读脚本的话，
那么这个脚本就会被杀死，
服务器就会回到正常状态。
